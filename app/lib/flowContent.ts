...
export const generateMovieNodes = (movies:[]) => {
    return movies.map((movie, i) => { 
        return { id: `mov-${movie}`, type: 'movieCustom', data: { id: movie }, position: { x: 200 + i * 600, y: 500 } }
    })
}

export const generateStarshipsNodes = (starships:[]) => {
...
export const generateHeroMovieEdges = (movies:[]) => {
    return movies.map((item, i) => {
        return { id: `hm-${item}`, source: 'hero', target: `mov-${item}`, type: 'default' }
    })
}

export const generateMovieShipEdges = (movArr:[], shipArr:[]) => {
    const edges = [];
